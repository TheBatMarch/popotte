/*
  # Ajouter des données d'exemple pour l'application

  1. Nouvelles tables de données
    - Catégories d'exemple (plats principaux, boissons, desserts, entrées)
    - Produits d'exemple avec images et prix
    - Articles de news d'exemple

  2. Sécurité
    - Utilise des vérifications d'existence pour éviter les doublons
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM products WHERE name = 'Makroud') THEN
    INSERT INTO products (name, description, price, category_id, image_url, is_available) 
    VALUES ('Gâteaux de semoule aux dattes', 'Makroud', 4.50, cat_desserts_id, 'https://images.pexels.com/photos/1126359/pexels-photo-1126359.jpeg', true);
  END IF;
END $$;
