...
export async function initializeDatabase() {
  try {
    ...
    // Vérifier si les tables existent et ont du contenu
    ...
    if (categoriesError) {
      console.log('❌ Erreur lors de la vérification des catégories:', categoriesError)
    ...
    if (!categories || categories.length === 0) {
      console.log('📦 Tables vides détectées, insertion des données...')
      await seedData()
    } else {
...
async function createTablesAndSeedData() {
  ...
  // Créer les tables avec SQL brut
  ...
  if (createError) {
    ...
    // Continuer même en cas d'erreur car les tables peuvent déjà exister
  }

  console.log('✅ Tables créées, insertion des données...')
  await seedData()
}
...
Merci de votre compréhension !`,
    ...
    if (newsError) {
      ...
      console.log('✅ Actualités insérées:', news?.length || 0)
    }
